    ...
    <div class="big">
        ...
        <div class="right">
            ...
            <table class="tb04" id="TId">
                <tr>
                	<th class="w155">图书编码</th>
                    <th class="w130">图书名称</th>
                    <th class="w130">图书作者</th>
                    <th class="w120">图书状态</th>
                    <th class="w100">是否新书</th>
                    <th class="w110">操作</th>
                </tr>
               <s:iterator value="upBooks" var="up">
                <tr>
                    <td>
                        <span class="w40">${up.BNum }</span>
                    </td>
                    <td>
                        <span class="">${up.BName }</span>
                    </td>
                    <td>
                        <span class="">${up.BAu }</span>
                    </td>
                    <td>
                        <span class="">${up.BStatus }</span>
                    </td>
                    <td>
                        <span class="w40">${up.isNew }</span>
                    </td>
                   
                    <td>
                    <c:if test="${up.BStatus eq '可借'}">
                     	<a href="<%=basePath%>bookAction!toNewLogin.action?bookId=${up.bookId}&isJy=借出"><span class="w40">借书</span></a>
                    </c:if>
                    <c:if test="${up.BStatus eq '借出'}">
                     	<a href="<%=basePath%>bookAction!toNewLogin.action?bookId=${up.bookId}&isJy=预定"><span class="w40">预定</span></a>
                    </c:if>
                     <c:if test="${up.BStatus eq '锁定'}">
                     	<span class="w40">锁定</span>
                    </c:if>
                    </td>
                </tr>
               </s:iterator>
